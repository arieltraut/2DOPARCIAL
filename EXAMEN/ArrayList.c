...
ArrayList* al_newArrayList(void)
{
    ...
    if(this != NULL)
    {
        ...
        if(pElements != NULL)
        {
            this->size=0; //al principio es cero
            this->pElements=pElements; //puntero a algun lado
            this->reservedSize=AL_INITIAL_VALUE; //reserva inicial
            this->add=al_add;
            this->len=al_len;
            this->set=al_set;
...
int al_add(ArrayList* this, void* pElement)
{
    ...
    if (this != NULL && pElement != NULL)
    {
        ...
        *((this->pElements)+(this->size)) = pElement;
        //this->pElements[this->size] = = pElement; es equivalente
        this->size++;
...
int al_clear(ArrayList* this)
{
    ...
    if(this != NULL)
    {
        //free (this->pElements);    nunca hacer free , hacer funcion resize down que vuelva al tamaño original (en este caso 10)
        this->size = 0;
        resizeDown(this);
        returnAux = 0;
    }
...
{
    int returnAux = -1;
    if (this != NULL && pElement != NULL && index >= 0 && index <= this->len(this)) //ver tema >= this->size
    {
        expand(this,index);
...
ArrayList* al_subList(ArrayList* this,int from,int to)
{
    ...
    if (this != NULL && from >= 0 && from <= this->len(this) && to >= 0 && to <= this->len(this) && from <= to)
    {
         ...
         if (newThis != NULL)
         {
             for (i=from ; i<=to ; i++)
             {
                newElement = this->get(this, i);
                newThis->add(newThis,newElement);
             }
             returnAux = newThis;
         }
    }
...
int al_map(ArrayList* this, void (*pFunc)(void*))
{
    int i,retorno = -1;
    void* aux;
    if (this != NULL && pFunc != NULL && aux != NULL)
    {
        for(i=0;i<this->len(this);i++)
        {
            aux = this->get(this,i);
...
{
    int returnAux = -1;
    void* auxiliar;//realloc siempre hay que crear un auxiliar a void
    if(this != NULL)
    {
...
/**
 * \brief Crea un nuevo ArrayList con elementos filtrados del arrayList original
 ...
 *
 */
ArrayList* al_filter(ArrayList* listIn, int (*funcionFilter)(void*))
{
    ArrayList* listaFiltrada = al_newArrayList();
    int i,retorno = NULL;
    void* aux;
    if (listIn != NULL && funcionFilter != NULL && aux != NULL)
    {
        for(i=0;i<listIn->len(listIn);i++)
        {
            aux = listIn->get(listIn,i);
          if((funcionFilter(aux))==1)
            {
                al_add(listaFiltrada,aux);
            }
        }
